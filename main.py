...
def main():
    ...
    board_img = pygame.transform.smoothscale(board_img, (board_img.get_width() * board_scale, board_img.get_height() * board_scale))
        ...
        for y in range(4):
            for x in range(8):
                if board.position[segment][y][x] is not None:
                    ...
                    pieces.append(Piece(piece_id[0], Position(segment, (x, y)), piece_pixel_pos, piece_id[1]))
    ...
    while True:
        ...
        for piece in pieces:
            ...
            if piece.rect.collidepoint((mouse_x, mouse_y)):
                ...
                if left_click:
                    ...
                    selected_piece_moves = piece_movegen(board, piece.position, piece.colour)
            else:
        ...
        if selected_piece is not None:
            for move in selected_piece_moves:
                move_polygon_points = board_polygons[int(move.segment)][int(move.square.y)][int(move.square.x)]
                ...
                if point.within(move_polygon):
                    pygame.draw.polygon(WINDOW, (255, 255, 255), move_polygon_points, width=5)
