...
def compute_polygons():
    ...
    for segment in range(3):
        segment_polygons = [[[] for x in range(8)] for y in range(4)]  # 8x4 array of polygons for the segment
        for half_segment in range(2):
            ...
            for point in half_segment_polygon:
                rotated_half_segment_polygon.append(rotate(half_segment_polygon[1], point, angle))
                    ...
                    for i in range(2):  # Offset corners for polygon
                        for j in range(2):
                            ...
                            bottom_lerp = lerp_vector(rotated_half_segment_polygon[3 - 3 * half_segment],
                                                      ...
                                                      (x + i) * 0.25)
                            lerp_point = lerp_vector(top_lerp, bottom_lerp, (y + j) * 0.25)
...
def handle_polygon_resize(polygons, new_scale, margin):
    polygons_copy = [[[[] for x in range(8)] for y in range(4)] for s in range(3)]
    for segment in range(3):
        for y in range(4):
            for x in range(8):
                new_point = []
                for point in polygons[segment][y][x]:
                    new_point.append((point[0] * new_scale + margin, point[1] * new_scale))
